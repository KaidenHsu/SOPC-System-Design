...
module VMM (
    ...
    input signed [`NUM_MPLIERS-1 : 0][`DWIDTH-1 : 0] rdata, // remember to fall back to using plain vector if SV isn't allowed
...
// shadow register for vec to save wasted cycles shifting back to its origianl state for every matrix column
...
// FSM
localparam S_IDLE = 0; // capture M_buf, N_buf when start is asserted
localparam S_WAIT_SRAM = 1; // wait for new raddr to arrive at the SRAM's input
localparam S_RD_VEC = 2; // read vec and vec_shadow
localparam S_VMM = 3; // MAC `NUM_MPLIERS elements at once, until a column is finished
...
always @(posedge clk, posedge rst) begin
    if (rst) raddr <= 0;
        ...
        case (state)
            ...
            S_RD_VEC: begin
                if (vec_counter < M_buf[$clog2(`MAX_M)-1 : $clog2(`NUM_MPLIERS)]-1 ||
                    vec_counter == ($clog2(`MAX_M/`NUM_MPLIERS))'(`MAX_M/`NUM_MPLIERS-1))
                    raddr <= raddr+1;
            end
        ...
        case (state)
            ...
            S_RD_VEC: begin
                if (vec_counter < M_buf[$clog2(`MAX_M)-1 : $clog2(`NUM_MPLIERS)]) begin
                    vec <= {rdata, vec[`MAX_M-1 : `NUM_MPLIERS]};
                    vec_shadow <= {rdata, vec_shadow[`MAX_M-1 : `NUM_MPLIERS]};
                end else begin
            ...
            S_VMM: begin
                if (row_counter == 0) vec <= vec_shadow; // shadow registers save wasted cycles
    ...
    case (state)
        S_VMM: begin
            ...
            for (i = 0; i < `NUM_MPLIERS; i=i+1) begin
                next_wdata = next_wdata + vec[i] * rdata[i];
            end
        end
